  ...
  return (
    <div className="splash-bg min-h-screen min-h-[100dvh] flex flex-col items-center text-white relative overflow-hidden select-none">
      ...
      <div className="splash-animate splash-delay-1 flex items-center justify-center gap-4 pt-[max(env(safe-area-inset-top,16px),16px)] mt-6 px-6 w-full max-w-sm">
        <OsmaniaLogo />
        <div className="w-px h-8 bg-gradient-to-b from-transparent via-emerald-300/60 to-transparent" />
        <OTBILogo />
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-5 px-6 -mt-4">
        ...
        <div className="splash-animate splash-delay-2 flex items-center gap-3">
          <span className="flex gap-1">
            ...
            <span className="w-6 h-px bg-gradient-to-r from-transparent to-emerald-300/55 self-center" />
          </span>
          <span className="text-sm tracking-[0.18em] text-white/70 uppercase font-light">
            {t("welcomeTo")}
          </span>
          <span className="flex gap-1">
            <span className="w-6 h-px bg-gradient-to-l from-transparent to-emerald-300/55 self-center" />
            <span className="w-1 h-1 rounded-full bg-emerald-300/70" />
          </span>
        </div>
        ...
        <h1 className="splash-animate splash-delay-2 text-center">
          ...
          <span className="text-4xl sm:text-5xl font-bold text-emerald-200">
            {t("bankHub").split(" ")[1] || ""}
          </span>
        </h1>
        ...
        <div className="splash-animate splash-delay-3 flex items-center gap-3 my-1">
          <span className="w-12 h-px bg-gradient-to-r from-transparent to-emerald-300/40" />
          <Sparkles className="w-4 h-4 text-emerald-200/80" strokeWidth={1.5} />
          <span className="w-12 h-px bg-gradient-to-l from-transparent to-emerald-300/40" />
        </div>

        {/* Premium Icon Card */}
        <div className="splash-animate splash-delay-3 splash-icon-card relative">
          <div className="absolute inset-0 rounded-[22px] border border-emerald-200/20 shadow-[0_24px_70px_-44px_rgba(16,185,129,0.7)]" />
          <div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-[20px] bg-white/[0.04] backdrop-blur-xl border border-white/[0.08] flex items-center justify-center">
            <div className="absolute w-14 h-14 sm:w-16 sm:h-16 rounded-[18px] border border-emerald-200/25" />
            <Landmark
      ...
      <div className="flex flex-col items-center gap-5 pb-[max(env(safe-area-inset-bottom,20px),20px)] mb-2">
        ...
        <div className="flex gap-1.5">
          {[0, 1, 2].map((i) => (
            <span
              key={i}
              className="w-1.5 h-1.5 rounded-full bg-emerald-200/70 splash-dot"
